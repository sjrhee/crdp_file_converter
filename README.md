...
- `--output`: 출력 파일 경로 (기본: `{e/d}{nn}_{원본파일명}.{확장자}`)
  ...
  - `{nn}`: 01부터 시작하는 두 자리 숫자 (중복 시 자동 증가)
  - 예: `e01_data.csv`, `d01_data.csv`, `e02_data.csv` 등
- `--batch-size`: Bulk API 배치 크기 (기본: `100`)
- `--host`: CRDP 호스트 (기본: `192.168.0.231`)
- `--port`: CRDP 포트 (기본: `32082`)
- `--policy`: 보호 정책 (기본: `P03`)
- `--timeout`: 요청 타임아웃 초 단위 (기본: `5`)
